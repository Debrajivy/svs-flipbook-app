...
function PageSheet({ data, pageNumber }: { data?: PageData; pageNumber: number }) {
  ...
  return <div className={`paper story ${data.paragraphs.length||data.images.length ? "" : "empty"} ${data.images.length===1?"portrait-story":""}`}><small>{data.continuation ? `Continued · ${String(data.serial).padStart(2, "0")}` : `Feature · ${String(data.serial).padStart(2, "0")}`}</small><h1>{data.article.title}</h1><i className="rule" />{data.paragraphs.length > 0 && <section className="copy">{data.paragraphs.map((paragraph, index) => <p key={index}>{paragraph}</p>)}{data.article.result && !data.continuation && <p className="result">{data.article.result}</p>}</section>}{data.images.length>0&&<section className={`story-gallery gallery-${data.images.length}`}>{data.images.map((src,index)=><figure key={src}><Image src={src} alt={`${data.article.title} — photograph ${index+1}`} fill sizes="(max-width: 700px) 84vw, 38vw" /></figure>)}</section>}<Footer n={pageNumber} /></div>;
}
